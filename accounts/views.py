...
class ProfileView(FieldsProfileMixin, UpdateView):
    ...
    def form_invalid(self, form):
        for field, errors in form.errors.items():
            messages.error(self.request, f"خطا در ویرایش اطلاعات {field}: {', '.join(errors)}")
...
class ExpertUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        default_province_id = self.object.province.id if self.object.province else ''
        default_city_id = self.object.city.id if self.object.city else ''
        default_health_center_id = self.object.health_center.id if self.object.health_center else ''
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            for field, errors in field_errors.items():
                ...
                messages.error(request, f'خطا در فیلد {field}: {"، ".join(error_messages)}')
...
class BehvarzRegisterView(CreateView):
    def get(self, request):
        ...
        return render(request, 'accounts/register_behvarz.html', {
            'form': form,
            'provinces': provinces,
            'cities': cities,
            'health_centers': health_centers,
...
class VillageUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(reverse_lazy('village_add_list', kwargs={'pk': health_center_id}))

    def get_success_url(self):
        if 'HTTP_REFERER' in self.request.META:
            return self.request.META['HTTP_REFERER']
        else:
            return reverse_lazy('village_add_list', kwargs={'pk': self.object.health_center.id})
...
def village_add_list_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Village.objects.filter(name=name, health_center=health_center.id).exists():
                messages.warning(request, f' این روستا قبلا برای این مرکز ثبت شده است.')
            else:
                new_village = Village.objects.create(name=name, health_center=health_center)
                new_village.save()
                messages.success(request, f'روستای "{name}" با موفقیت ثبت شد. ')
                return HttpResponseRedirect(reverse('village_add_list', args=[health_center.id]))
        else:
            form = VillageForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if HealthCenter.objects.filter(name=name, city=city.id).exists():
                messages.warning(request, f'نام "{{name}}" قبلاً برای این شهر وارد شده است.')
            else:
...
class CityUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(reverse_lazy('city_add_list', kwargs={'pk': province_id}))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if City.objects.filter(name=name, province=province.id).exists():
                ...
                return HttpResponseRedirect(reverse('city_add_list', args=[province.id]))
    else:
        form = CityCreateForm()
